...
20200903: [MV] Reversing the competition (to be able to submit datasets)
            - retrieve the solution file in the prediction dir (output dir of ingestion),
              after ingestion has actually copied it in the pred directory (so it's now retrieved in 
              the first successful call of score_new_predictions rather than in the initializer of the Evaluator class)
...
class Evaluator(object):
  ...
  def __init__(self, solution_dir=None, prediction_dir=None, score_dir=None,
    ...
    #change it to False when a copy of the .solution file is made by ingestion
    self.no_solution_yet = True
    # Resolve info from directories (see score_new_predictions) (later)
    self.solution = None
  ...
  def get_solution_or_None(self):
    """Get solution as NumPy array from `self.solution_dir` or None if no solution file has been
     produced by ingestion yet"""
  ...
  def score_new_predictions(self):
    #If the solution has yet to be produced by ingestion
...
if __name__ == "__main__":
    ...
    logger.debug("-" * 50)
    solution_dir = args.prediction_dir #the solution file has been copied in the prediction dir

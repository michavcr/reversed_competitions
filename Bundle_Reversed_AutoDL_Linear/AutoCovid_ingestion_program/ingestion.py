...
Previous updates:
20200903: [MV] Reversing the competition (in order to submit datasets instead of code).
               - switching the program arguments dataset_dir and code_dir
               - copying the .solution file (submitted by the participant) with copy_solution in output_dir 
               so that the scoring program has access to it.
...
def copy_solution(dataset_dir, output_dir):
  """Copy the .solution file from the dataset directory (submitted by the participant in this
  reversed competition) to the prediction directory, so that the scoring program has access to it."""
...
if __name__=="__main__":
    ...
    logger.debug("-" * 50)
    dataset_dir = args.code_dir #SWITCH CODE AND DATASET
    output_dir = args.output_dir
    ingestion_program_dir= args.ingestion_program_dir
    code_dir= args.dataset_dir #SWITCH CODE AND DATASET
